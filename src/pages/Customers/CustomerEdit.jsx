...
const CustomerEdit = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      await CustomerService.getCustomerById(id).then((response) => {
  ...
  const onSave = async () => {
    ...
    await CustomerService.updateCustomer(id, customer).then(
      (response) => {
        console.log(response);
        navigate("/customers");
      },
    ...
    <>
      <div className=" md:m-10 p-1 md:p-10 bg-white rounded-3xl">
        <div className="flex justify-between items-center mb-6">
          <Header
            title={`Khách hàng ${data.firstName}`}
            category="Chỉnh sửa thông tin khách hàng"
          />
        </div>
                        ...
                        <div>
                          <label className="block text-sm font-medium text-gray-700">
                            <div className="mt-1">
                              ...
                              <h1 className="font-bold mb-1">Avatar </h1>
