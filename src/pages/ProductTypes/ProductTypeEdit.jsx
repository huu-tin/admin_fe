...
const ProductTypeEdit = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      await ProductTypeService.getProductTypeById(id).then((response) => {
  ...
  const onSave = async () => {
    ...
    await ProductTypeService.updateProductType(id, producttype).then(
      (response) => {
        console.log(response);
    ...
    <>
      <div className=" md:m-10 p-1 md:p-10 bg-white rounded-3xl">
        <div className="flex justify-between items-center mb-6">
          <Header
            title={`Loại sản phẩm ${data?.name}`}
            category="Chỉnh sửa thông tin loại sản phẩm"
          />
        </div>
